...
В данном проекте решена задача сегментации повреждений дорожного покрытия. Для этого использовались свёрточные нейросети. 
Также реализован алгоритм опредления размера повреждения.


<h2>Запуск</h2>
Для подготовки данных для обучения или представления результатов моделей используется модуль <strong> Preprocessor из Data_Preprocessor </strong>. Исходными данными для обучения являются изображения и соответствующие им карты сегментации. Для представления результатов достаточно набора изображений или видеоотрывка.

Для проведения аугментации используется модуль <strong> Augmentation из Data_Preprocessor </strong>. Исходыми данными являются изображения и соответствующие им карты сегментации.

Для обучения моделей сегментации дорожного покрытия используется модуль <strong> Road_models </strong>, где нужно выбрать какую из моделей необходимо изучить unet или deeplab.

Для обучения модели сегментации дефектов дорожного покрытия используется модуль <strong> Defect_model </strong>.

Для оценки качества работы модели используется модуль <strong> Evaluate_and_Inference </strong>. Исходными данными являются изображения и соответствующие им карты сегментации. Для расчёта метрик необходимо поставить значение metric=True. В итоге будут оценены метрики качества (IoU, Weighted Pixel Accuracy,
Precision, Recall, Dice index, Pixel Accuracy, Confusion matrix) для изображений в отдельности или для всего набора, если global_metric=True.

Для представления результатов используется модуль <strong> Evaluate_and_Inference </strong>. Достаточно только изображений, значение metric может быть False, тогда метрики не будут рассчитываться. В итоге будут, при необходимости, выводиться предсказания модели на каждом изображении, а также они будут сохранены в указанную дирректорию.

Для определения площади повреждений используется модуль <strong> Defect_area </strong>. Исходными данными является карта сегментации, для которой будет рассчитываться площадь и изображения для определения пропорций для рассчётов.
...
разрушения дорожного покрытия. Набор изображений разделён на две части. В первой части разметка производилась на два класса, дорога и фон. Во второй — повреждение и фон.
...
<ul>
  ...
  Класс Preprocessor используется для преобразования исходных изображений в подходящий для нейросети вид. Изображения обрезаются, уменьшаются и сохраняются в формате .png и .npy(преобразование в np.array и сохранение на этом этапе позволяет избежать этого на последующих этапах, что ускоряет обучение сетей). Также можно преобразовать видео. 
  Также включён модуль Augmentator для аугментации данных.
  <li>Data_Generator</li>
  Используется для подачи данных нейросети небольшим пакетами.
  <li>Callbacks_lib</li>
  ...
  Класс DeeplabV3_mv2_model для создания и обучения модели Deeplab v3 с энкодером mobilenet v2 для сегментации дорожного покрытия.
  <li>Defect_models</li>
  ...
  рассчитать метрики качества для любой из подзадач - сегменации дорожного покрытия или повреждений на нём,
  при чём может быть использованно любое число моделей с любой степенью важности каждой. Метрики рассчитываются
  как для отдельного изображения, так и средние значения для всего набора,
  строятся графики изменения метрик на обучающей и валидационной выборке.
  Также можно создать видео, отображающее предсказания модели.
  Для отдельного изображения можно получить карту активации.
  <li>Defect_area</li>
  Класс определения площади повреждения дорожного покрытия.
 <li>Filtres</li>
...
<table border="1" width="100%" cellpadding="5" bgcolor="black" cols = 2>
 ...
 Пример изображения, сегментации дороги на нём с помощью нейросети с архитектурой deeplab v3 + mobilenet v2 и с помощью нейросети с архитектурой unet, а также сегментация с использованием ансамблирования взвешенным средним этих двух архитектур (сверху вниз)
...
мини-батчам.
<p>На графиках представлено изменение функции потерь (верхний график) и индекса Дайса (нижний график) на обучающем и валидационном наборах в зависимости от эпохи.</p>
