...
Для обучения модели сегментации дефектов дорожного покрытия используется модуль <strong> Defect_model </strong>.
 ...
 Для оценки качества работы модели исползуется модуль <strong> Evaluate_and_Inference </strong>. Исходными данными являются изображения и соответствующие им карты сегментации, а также поставить значение metric=True.В итоге будут оценены метрики качества (IoU, Weighted Pixel Accuracy,
...
Для представления результатов используется модуль <strong> Evaluate_and_Inference </strong>. Достаточно только изображений, значение metric может быть False, тогда метрику не будут расситываться. В итоге будут, при необходимости, выводиться предсказания модели на каждом изображении, а также они будут сохранены в указанную дирректорию.

 Для определения площади повреждений используетс модуль <strong> Defect_area </strong>. Исхдными данными являются карта сегментации, для которой будет рассчитываться площадь и изображения для определения пропорций для рассчётов.
